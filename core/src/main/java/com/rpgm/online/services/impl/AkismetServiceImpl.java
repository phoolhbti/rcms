...
import com.rpgm.online.services.HttpService;
import com.rpgm.online.services.LinkRewriterService;
import com.rpgm.online.services.OsgiConfigurationService;
...
public class AkismetServiceImpl implements AkismetService {
    ...
    private static final String COMPONENT_PID = "rpgm Akismet settings";
     ...
     *
     * @param commentResource The rpgm:comment resource to act upon.
    ...
    public boolean isSpam(final Resource commentResource) {
        ...
        if (result) {
            try {
                ...
                properties.put(RPGMConstants.COMMENT_PROPERTY_SPAM, true);
    ...
    public boolean submitSpam(final Resource commentResource) {
        ...
        if (result) {
            try {
                final ModifiableValueMap properties = commentResource.adaptTo(ModifiableValueMap.class);
                properties.put(RPGMConstants.COMMENT_PROPERTY_SPAM, true);
                properties.put(RPGMConstants.COMMENT_PROPERTY_DISPLAY, false);
                commentResource.getResourceResolver().commit();
            } catch (PersistenceException e) {
                LOGGER.error("Could not save spam properties", e);
            }
    ...
    public boolean submitHam(final Resource commentResource) {
        ...
        if (result) {
            try {
                final ModifiableValueMap properties = commentResource.adaptTo(ModifiableValueMap.class);
                properties.put(RPGMConstants.COMMENT_PROPERTY_SPAM, false);
                properties.put(RPGMConstants.COMMENT_PROPERTY_DISPLAY, true);
                commentResource.getResourceResolver().commit();
            } catch (PersistenceException e) {
                LOGGER.error("Could not save spam properties", e);
            }
    ...
    /**
     * Get an AkismetComment from a rpgm:comment resource.
     *
     * @param commentResource The rpgm:comment resource.
    ...
    private AkismetComment getAkismetComment(final Resource commentResource) {
        ...
        if (commentResource != null) {
            ...
            akismetComment.setUserIp(properties.get(RPGMConstants.COMMENT_PROPERTY_USER_IP, String.class));
            akismetComment.setUserAgent(properties.get(RPGMConstants.COMMENT_PROPERTY_USER_AGENT, String.class));
            akismetComment.setReferrer(properties.get(RPGMConstants.COMMENT_PROPERTY_REFERRER, String.class));
            akismetComment.setPermalink(externalLink);
            akismetComment.setCommentType(AKISMET_COMMENT_TYPE);
            akismetComment.setCommentAuthor(properties.get(RPGMConstants.COMMENT_PROPERTY_AUTHOR, String.class));
            akismetComment.setCommentContent(properties.get(RPGMConstants.COMMENT_PROPERTY_COMMENT, String.class));
        }
