 ...
 */
@SlingServlet(paths = RPGMConstants.SERVLET_PATH_PUBLIC + "/addcomment")
        ...
        if (!recaptchaService.getEnabled() || recaptchaService.validate(request)) {
            ...
            try {
                ...
                String parentPath = StringUtils.isNotBlank(commentPath)
                                    ? commentPath
                                    : blogPath.replace(RPGMConstants.BLOG_PATH, RPGMConstants.COMMENTS_PATH);
                ...
                properties.put(RPGMConstants.COMMENT_PROPERTY_AUTHOR, author);
                properties.put(RPGMConstants.COMMENT_PROPERTY_COMMENT, comment);
                properties.put(JcrConstants.JCR_PRIMARYTYPE, RPGMConstants.NODE_TYPE_COMMENT);
                properties.put(RPGMConstants.COMMENT_PROPERTY_USER_AGENT, httpService.getUserAgent(request));
                properties.put(RPGMConstants.COMMENT_PROPERTY_USER_IP, httpService.getIPAddress(request));
                properties.put(RPGMConstants.COMMENT_PROPERTY_REFERRER, httpService.getReferrer(request));
                properties.put(RPGMConstants.COMMENT_PROPERTY_DISPLAY, true);
                properties.put(RPGMConstants.COMMENT_PROPERTY_EDITED, false);
