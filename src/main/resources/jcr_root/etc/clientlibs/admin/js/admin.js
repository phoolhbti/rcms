$(function() {
  ...
  $('.logout').click(function(e){
    ...
    $.post('j_security_check', {
      ...
      if (data.status === 403) {
        window.location = '/content/rpgm/login.html'
      }
    });
  });
});
...
app.controller('AssetController', function($scope, $http, Upload) {
    ...
    $scope.currentPath = '/content/rpgm/assets';
    $scope.folders = [];
    $scope.assets = [];
    ...
    $scope.navigate = function(folder, isRelative) {
      ...
      $scope.currentPath = '/content/rpgm/' + $scope.breadcrumbs.join('/');
...
app.factory('SettingsService', function($http, formDataObject) {
  var settingsFactory = {},
      PATH_BASE = '/bin/admin',
      PATHS = {
        system    : '/content/rpgm/admin.config.html',
